...
Full run time: `02_main.ipynb` runs end-to-end in ~7 minutes on cached data. The one-time classification step (`scripts/run_production.py`) cost ~$23 and ran ~9–18 h for all 79,372 articles; the cost log is committed to `data/processed/llm_cost.csv` so it does not need re-running to read the notebook.
