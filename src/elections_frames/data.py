...
The English-relevance filter and dedup happen in Session 3 (``cleaning.py``) —
...
# memory on a full corpus by >50%; downstream sessions can re-load with a wider
...
def outlet_provenance_join(
    ...
    Pragmatic Session-2 implementation: simple bare-domain substring match.
    Unmatched rows get ``outlet_origin = "Unknown"``. Session 3's outlet
