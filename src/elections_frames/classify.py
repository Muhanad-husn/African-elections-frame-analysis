...
Provider (Session 6 migration): the **active** provider is OpenRouter
...
rate (Session 4), which made eval-scale passes (~20 h single-threaded) and the
...
# Active provider for the production run (Session 6 migration: NVIDIA -> OpenRouter).
