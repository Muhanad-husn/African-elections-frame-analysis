...
``notebooks/02_main.ipynb`` (Session 3) — read the notebook for the rationale
...
def attribute_outlet_origin(
    ...
    Rule (refined Session-3 version):
...
def filter_relevant(
    ...
    - ``"keyword"`` — URL/names/themes/source contains one of the election's
      ...
      40-70% false-positive recall during the Session-3 probe). Election-specific.
    - ``"hybrid"`` — theme AND keyword. Highest precision; default.

    See the Session-3 relevance-filter decision block in ``02_main.ipynb``.
    """
...
def build_text_snippet(row: pd.Series) -> str:
    ...
    Per Session-3 decision (Decision Log #1): we do not scrape live HTML.
